...
class DocumentField extends Forms\Components\Field
{
                        ...
                        ->afterStateUpdated(function (mixed $state, Forms\Set $set) {
                            if ($state['type'] === \App\DocumentType::Invoice->value) {
                                if ($state['documentable_type'] === 'purchase' && ! is_null($state['documentable_id'])) {
                                    ...
                                    if ($purchase->document()->where('type', 'invoice')->exists() || $purchase->tendered >= $purchase->total_cost || $purchase->payment_status === \App\PaymentStatus::Paid->value) {
                                        $set('amount', null);
                                        $set('amount_paid', null);
                                        $set('created_at', null);
                                        $set('payment_status', null);
                                    ...
                                    if ($sale->document()->where('type', 'invoice')->exists() || $sale->tendered >= $sale->total_price || $sale->payment_status === \App\PaymentStatus::Paid->value) {
                                        $set('amount', null);
                                        $set('amount_paid', null);
                                        $set('created_at', null);
                                        $set('payment_status', null);
                                ...
                                if ($state['documentable_type'] === 'purchase' && ! is_null($state['documentable_id'])) {
                                    ...
                                    if ($purchase->document()->where('type', 'receipt')->exists() || $purchase->tendered < $purchase->total_cost || $purchase->payment_status !== \App\PaymentStatus::Paid->value) {
                                        $set('amount', null);
                                        $set('amount_paid', null);
                                        $set('created_at', null);
                                        $set('payment_status', null);
                                    ...
                                    if ($sale->document()->where('type', 'receipt')->exists() || $sale->tendered < $sale->total_price || $sale->payment_status !== \App\PaymentStatus::Paid->value) {
                                        $set('amount', null);
                                        $set('amount_paid', null);
                                        $set('created_at', null);
                                        $set('payment_status', null);
