        ...
        $base = [
            Forms\Components\Section::make([
                Forms\Components\Select::make('customer_id')
                    ->relationship('customer', 'name', fn (Builder $query) => $query->where('role_id', Role::CUSTOMER))
                    ->searchable()
                    ->preload()
                    ->disabled(fn (string $operation): bool => $operation === 'edit')
                    ->manageOptionForm(CustomerLayout::getForm())
                    ->fillEditOptionActionFormUsing(function (mixed $state) {
                        return Customer::query()->find($state)->load('customerInfo')->toArray();
                    })
                    ->updateOptionUsing(function (array $data, Forms\Form $form) {
                        ...
                        $form->getRecord()?->load('customerInfo')->customerInfo->update($customer_info);
                    }),
            ...
            Forms\Components\Repeater::make('productSales')
                ...
                ->afterStateUpdated(function (Forms\Get $get, Forms\Set $set, $state) {
                    ...
                    $set('payment_status', \App\Enums\PaymentStatus::Paid->value);
                })
                ->schema([
                    Forms\Components\Select::make('product_id')
                        ...
                        ->manageOptionForm(ProductLayout::getForm()),
                        ...
                        ->schema([
                            Forms\Components\TextInput::make('discount')
                                ...
                                ->default(function () {
                                    ...
                                    if (isset($user->user_id)) {
                                        return $user->query()->find($user->user_id)->load('setting')->setting->discount;
                                    }

                                    return $user->load('setting')->setting->discount;
                                })
                                ->minValue(0)
                                ->maxValue(42949672.95)
                                ->live(true)
                            ...
                            Forms\Components\TextInput::make('vat')
                                ...
                                ->default(function () {
                                    ...
                                    if (isset($user->user_id)) {
                                        return $user->query()->find($user->user_id)->load('setting')->setting->vat;
                                    }

                                    return $user->load('setting')->setting->vat;
                                })
                                ->disabled()
                                ->dehydrated()
                                ->minValue(0)
                            ...
                            Forms\Components\TextInput::make('tendered')
                                ...
                                ->afterStateUpdated(function (Forms\Get $get, Forms\Set $set, $state) {
                                    ...
                                    if ($state >= $get('total_price')) {
                                        ...
                                        $set('payment_status', \App\Enums\PaymentStatus::Pending->value);
                                    }
                                }),
                    ...
                    Forms\Components\Section::make('Payment Info')
                        ...
                        ->schema([
                            ...
                            Forms\Components\DatePicker::make('document.due_date')
                                ...
                                ->disabled(fn (Forms\Get $get): bool => $get('payment_status') === \App\Enums\PaymentStatus::Paid->value)
                                ->dehydrated(fn (Forms\Get $get): bool => $get('payment_status') === \App\Enums\PaymentStatus::Pending->value)
                                ->required(fn (string $operation) => $operation === 'create'),
